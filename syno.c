...
typedef struct {
    S_MotSynonymes *dico;  // Tableau de mots et synonymes
    int nb_mots;         // Nombre total de mots
} S_DictionnaireSynonymes;
...
S_DictionnaireSynonymes * charger_synonymes(const char *nom_fichier) 
{
FILE 	*fichier;
...
S_DictionnaireSynonymes * dico = malloc(sizeof(S_DictionnaireSynonymes));
if(dico == NULL) {
...
dico->dico =  malloc(dico->nb_mots * sizeof(S_MotSynonymes));
...
char *fct_synonyme(S_DictionnaireSynonymes * dico,const char *mot) 
{
int i,index;
...
S_DictionnaireSynonymes * dico = charger_synonymes(SYNONYMES);
if(dico == NULL) {
